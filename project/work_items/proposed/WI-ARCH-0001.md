...
acceptance:
  ...
  - scripts/smoke passes against the existing committed reference signatures (no visual regression)
...
**Maintainer decision (2026-07-30):** keep the crate name `webgpu_vector_lib` for now; the rename to `velumin` DP-0002 leaves open is deferred to a future item.
...
## Required Changes
1. Create `velumin-core/Cargo.toml` (new workspace member; `edition = "2024"`, `rust-version = "1.87"`; only dependency `bytemuck`).
...
- Do not rename `webgpu_vector_lib` to `velumin` (deferred per maintainer decision).
