...
acceptance:
  ...
  - All 8 swept negotiation references, plus focus/current_focus.md's separate "selected active workstream" exit-criterion statement, are consistent with the reorder and with Phase 2 now being complete rather than active, excluding historical decision_log.md and project/executions/ records
...
## Required Changes
1. In `project/design/proposals/proposed/DP-0002-cross-platform-renderer-architecture.md`'s Phase 2 section: narrow the "Add explicit adapter/capability negotiation for web and desktop" bullet to web-side only (mark it done, citing both halves that together constitute the completed web-side negotiation: adapter acquisition and unavailable-adapter handling in `WebGPU::create_with_preset` via `instance.request_adapter`, and capability validation — surface format, alpha mode, present mode, and limits — in `Renderer::new`, which receives the already-acquired adapter and does not request it itself. Do not cite `Renderer::new` alone, since it covers only the capability half, not adapter acquisition — `WI-ARCH-0003`'s own Problem/Context section documents this split explicitly. `Renderer` is currently a private struct and `project/memory/decision_log.md`'s renderer-crate-boundary question remains explicitly unresolved, so do not assert or imply how a future Phase 3 desktop caller will reach either function), and remove the desktop-side clause and the "Unresolved sequencing circularity" note, replacing the note with one sentence pointing to the new `project/memory/decision_log.md` entry.
