...
## Required Changes
1. Replace `struct RendererError(String)` (`webgpu_vector_lib/src/lib.rs:16`) with an enum covering all 7 current failure sites, each with its own variant (not a shared `Other(String)` catch-all): the 5 capability-negotiation sites in `Renderer::new` (`:375` unsupported surface format, `:378` unsupported alpha mode, `:384` missing FIFO present mode, `:391` insufficient limits, `:407` device request failed), plus 2 runtime sites in `Renderer::render` (`:608` surface texture unavailable — static message, no dynamic data; `:613` frame acquisition failed — wraps a `{:?}`-Debug-formatted status string supplied by the call site). Each variant's `Display` arm must own the full message text (the static wording, not just a passthrough of a pre-formatted string) — the call site should only ever need to supply the raw dynamic substitution value (e.g. `format!("{:?}", status)` for the status debug string), never construct any part of the surrounding sentence itself. This is what makes each variant's `Display` output independently testable without invoking `Renderer::render` (which needs a real GPU/wgpu setup, only exercised by `scripts/smoke`).
