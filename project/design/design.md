...
- `DP-0006 Vector CRT Renderer Migration` is adopted (2026-07-24) as the project's vector-display rendering direction and is partially implemented (`implementation_status: partial`): the fixed 4:3 viewport, additive multi-layer glow, and an internal display-preset set have landed alongside the demos, verified by code inspection (`EV-0008`) and browser visual-smoke capture (`EV-0009`). The `WI-SMOKE-0001` smoke check (`scripts/smoke`) now captures the scenes at 4:3, wide, and tall, covering the non-4:3 letterbox/pillarbox path. The public-preset-API question is settled by the adopted `DP-0007`; implementing it (and capturing the non-default presets) is tracked in `WI-PRESET-0001`.
- `DP-0007 Display Preset Public API` is adopted (2026-07-25) as the decision to expose DP-0006's display presets: a public, `#[non_exhaustive]` `VectorDisplayPreset` enum selectable at creation and at runtime (named presets only in v1; a public custom-settings API is deferred). Implementation is tracked in `WI-PRESET-0001` (`implementation_status: not_started`).
