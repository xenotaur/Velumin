...
- Add explicit adapter/capability negotiation for web and desktop. — web-side capability checks exist (missing/blocked WebGPU adapter handling); desktop-side negotiation does not exist (no native surface path at all — `WI-ARCH-0002` explicitly did not add this, since there is no native host to negotiate against yet). **Unresolved sequencing circularity, flagged 2026-08-01 during `WI-ARCH-0003`'s review:** this bullet is listed under Phase 2, but achieving it requires a native surface/adapter acquisition path — the thing Phase 3 (native `winit` shell) would build, and Phase 3 is sequenced *after* Phase 2 in this milestone list. As scoped today, this specific bullet cannot complete before Phase 3 exists, while Phase 3 does not depend on this bullet completing first. A future decision should either scope a minimal native-surface acquisition seam as its own Phase 2 slice (ahead of the full Phase 3 shell), or explicitly merge/reorder this bullet with Phase 3. Not resolved here.
