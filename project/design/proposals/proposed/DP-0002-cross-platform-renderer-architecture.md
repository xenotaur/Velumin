...
### Phase 2: Modern Shared `wgpu` Renderer — Done (browser-only)
- **Chosen for implementation on 2026-07-31 and completed for browser-only scope after the 2026-08-01 reordering** (`project/memory/decision_log.md`), following completion of `WI-ARCH-0001` — Phase 1's platform-neutral type-extraction slice, not all of Phase 1 (Phase 1 remains "Partially done"; the renderer/browser-adapter isolation bullet above is not started). Its concrete first slice, [WI-ARCH-0002](../../../work_items/resolved/WI-ARCH-0002.md) (PR #20), is done: `Renderer` is now surface-agnostic and host-buildable. See `project/memory/decision_log.md` for scope notes on how this phase interacts with Phase 1's deferred renderer/browser-adapter split.
- Complete the DP-0001 `wgpu` upgrade. — done (DP-0001 adopted).
- Introduce a reusable renderer state that can render to any supported `wgpu::Surface`. — done: `WI-ARCH-0002` (PR #20) made `Renderer` compile and construct on the host target from a generic `wgpu::Surface`/`Adapter`, with a platform-neutral error type and logging shim; `WebGPU` (the browser-specific canvas/adapter setup) stays `wasm32`-gated separately.
- Add explicit web-side adapter/capability negotiation. — done: `WebGPU::create_with_preset` owns browser adapter acquisition and unavailable-adapter handling via `instance.request_adapter`, while `Renderer::new` validates the acquired adapter and surface capabilities (surface format, alpha mode, present mode, and limits). The historical desktop-side sequencing circularity was resolved on 2026-08-01 by moving desktop-side negotiation to Phase 3; see `project/memory/decision_log.md`.
...
- Add explicit desktop-side adapter/capability negotiation. — not started; moved from Phase 2 on 2026-08-01 to resolve the sequencing circularity, and still requires Phase 3's own explicit selection before work begins.
