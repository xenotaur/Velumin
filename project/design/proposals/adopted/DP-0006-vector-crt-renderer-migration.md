...
Follow-up work under this adopted direction (tracked separately, not adoption prerequisites): visual verification of the non-4:3 resize/letterbox behavior at wide and tall browser sizes, whether the internal preset set becomes a selectable public API (`DP-0007`), visual capture of the non-default presets, and an automatable/committed screenshot smoke check (see `WI-SMOKE-0001`).
