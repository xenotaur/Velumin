...
- Decide whether/when to build DP-0007's deferred public custom display-settings API or the next DP-0008 follow-up. DP-0002 Phase 2 is complete for its browser-only shared-renderer scope; desktop-side adapter/capability negotiation is now a Phase 3 bullet and still awaits Phase 3's own workstream selection.
...
1. Decide whether the next API follow-up should extend DP-0008's immediate frame surface, define DP-0007 custom display settings, or begin a separate retained scene/material proposal.
2. Select and scope DP-0002 Phase 3 before pursuing desktop-side adapter/capability negotiation or a native `winit` shell.
3. Validate DP-0008 against a real downstream Replication Vector or Blasterites integration before expanding the public surface.
